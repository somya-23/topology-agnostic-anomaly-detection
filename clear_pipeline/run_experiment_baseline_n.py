"""run_experiment_baseline_n.py — Original TopoAR + linear baseline(N) preprocessing.

WHAT THIS ISOLATES
------------------
Exactly ONE change vs run_experiment.py:

  baseline(N) subtraction for CU cpu / mem_pct / mem_bytes BEFORE RobustScaler.

  For each of these three channels, a linear fit
      median_c(N) = alpha_c * N + beta_c
  is computed from the training topologies (N=2, N=3 → two points uniquely
  determine the line). At train time each topology's stream is centered by
  subtracting its own fitted baseline. At test time the fitted line is
  evaluated at the test topology's N_DU and subtracted before transform_stream.

  Everything else is identical to run_experiment.py:
    - net_tx / net_rx still divided by N_DU (physically: per-DU rate).
    - Model: CalibratedTopoAR (original attention architecture, unchanged).
    - Scaler: RobustScaler v0.
    - Training loop, calibration, closed-loop inference, evaluation: unchanged.

WHY THIS EXPERIMENT MATTERS
----------------------------
  From analyze_channel_shifts.py, the three channels with OOD cross-topology
  shift are cpu, mem_pct, mem_bytes:
    cu1_du2 (N=1) scaled position:  cpu=-0.98  mem_pct=-1.01  mem_bytes=-1.51
  These fall outside the pooled IQR of the training scaler (trained on N=2, N=3),
  causing elevated reconstruction error on ALL normal timesteps at the N=1 test
  topology. The current pipeline addresses this with the cold-start threshold
  probe (an ad-hoc band-aid). baseline(N) fixes it at the input level.

  Expected result: cu_shift ≈ 1.0 in the cold-start probe for the N=1 topology
  (cu1_du2), meaning the model's normal error distribution on the test topology
  matches the calibration distribution — no threshold inflation needed.

CHECKPOINT PREFIX: "bl_n_"
--------------------------
  Checkpoints are stored as  bl_n_model_ckpt_test_{topo}.pt
  Delete them to force a retrain after any preprocessing change.

RUN
---
    cd .../clear_pipeline
    python run_experiment_baseline_n.py
  (RUN_ALL_LOO = True by default)
"""
...
# =============================================================================
# USER INPUTS — same as run_experiment.py
# =============================================================================
...
# ── baseline(N) channels ──────────────────────────────────────────────────────
# Post-slice CU indices: 0=cpu, 1=mem_pct, 2=mem_bytes.
# Confirmed linear in N_DU with flat IQR by analyze_channel_shifts.py (res/IQR≈0).
CU_BASELINE_IDX = [0, 1, 2]
...
def slice_features(z: dict):
    """Identical to run_experiment.py — /N_DU on net_tx/rx, imputation, derived features."""
...
# =============================================================================
# BASELINE(N) HELPERS  — the only new preprocessing logic
# =============================================================================

def fit_cu_baseline(raw_streams: list, n_du_list: list):
    """Fit linear median_c(N) = alpha_c*N + beta_c for cpu/mem_pct/mem_bytes.

    raw_streams : list of {"cu": (T, cu_dim), ...}  post-slice, pre-scale arrays
    n_du_list   : list of int N_DU, same order as raw_streams

    Returns alpha, beta — shape (len(CU_BASELINE_IDX),) float64.

    With training topologies N=2 and N=3, two points uniquely determine the
    linear model. Extrapolation to N=1 is exact because res/IQR ≈ 0 for these
    channels (confirmed by analyze_channel_shifts.py).
    """
...
def apply_cu_baseline(cu: np.ndarray, n_du: int,
                      alpha: np.ndarray, beta: np.ndarray) -> np.ndarray:
    """Subtract fitted topology-level idle baseline from CU_BASELINE_IDX channels."""
    cu = cu.copy()
...
# =============================================================================
# PHASE 1: PREPROCESSING — baseline(N) fit + apply, then RobustScaler
# =============================================================================

def phase_preprocess(train_zs, train_topos, train_n_dus):
    """Fit RobustScaler on baseline-centered, pooled training streams.
    ...
    # Fit linear baseline from the training topologies
    alpha_bl, beta_bl = fit_cu_baseline(raw_streams, train_n_dus)
    ch_names = ["cpu", "mem_pct", "mem_bytes"]
    print("  baseline(N) fit (slope / intercept):")
    for i, nm in enumerate(ch_names):
        print(f"    {nm:12s}: slope={alpha_bl[i]:.4g}   intercept={beta_bl[i]:.4g}")

    # Center each training stream before the scaler sees it
...
# =============================================================================
# PHASE 2: TRAIN — original CalibratedTopoAR, unchanged
# =============================================================================
...
# =============================================================================
# PHASE 3 / 3b: INFERENCE — identical to run_experiment.py
# =============================================================================
...
# =============================================================================
# PHASE 4: EVALUATE — identical to run_experiment.py
# =============================================================================
...
def run_one(train_topos, test_topo):
    model_ckpt  = Path(f"bl_n_model_ckpt_test_{test_topo}.pt")
    cu_dim_info = len(np.arange(7)[CU_FEAT_SLICE])
    du_dim_info = len(np.arange(37)[DU_FEAT_SLICE])

    print(f"\n{'='*70}")
    print(f"  [Original TopoAR + baseline(N)]  {STRESS_NAMES[STRESS_TYPE]} stress detection")
    print(f"  Train : {train_topos}   Test : {test_topo}")
    print(f"  CU features: {cu_dim_info}   DU features: {du_dim_info}   Device: {DEVICE}")
    print(f"  Preprocessing: /N_DU for net  +  linear baseline(N) for cpu/mem")
    print(f"{'='*70}")
    ...
    if model_ckpt.exists():
        ...
        mismatches = []
        if ckpt.get("variant") != "baseline_n":
            mismatches.append(f"variant: expected baseline_n, got {ckpt.get('variant')}")
    ...
    # [4] Cal inference + calibrate thresholds
    ...
    # [6] Transform test topology — apply baseline with test N_DU
    ...
    # Subtract the train-fitted baseline at the test topology's N_DU
    ...
    print(f"  Baseline subtracted at N_DU={n_du_te}: "
          + "  ".join(f"ch{CU_BASELINE_IDX[i]}={alpha_bl[i]*n_du_te+beta_bl[i]:.3g}"
                      for i in range(len(CU_BASELINE_IDX))))

    # [6b] Cold-start probe — expect shift ≈ 1.0 after baseline correction
    ...
    if cu_shift < 1.5:
        print(f"  ✓ shift≈1 — baseline(N) successfully removed cross-topology level shift")
    else:
        print(f"  ✗ shift={cu_shift:.2f}x — residual shift remains; check other channels")
    ...
    if SAVE_ERRORS:
        feat_tag = f"f{cu_dim}"
        err_path = Path(f"recon_errors_bl_n_{test_topo}_{feat_tag}.npz")
        np.savez(err_path,
    ...
    # [8b] Per-channel normal sq-error comparison: before vs after baseline
    ...
    print("\n[8c] Per-channel CU mean sq-error: CAL vs TEST normal rows")
    ...
    print(f"  {'channel':>10s}  {'cal_mean':>10s}  {'te_normal':>10s}  {'ratio':>6s}  note")
    for c in range(cu_sqerr.shape[1]):
        ...
        note   = " ← baseline applied" if c in CU_BASELINE_IDX else ""
        print(f"  {feat_names[c]:>10s}  {cal_m:>10.4f}  {te_m:>10.4f}  {ratio:>6.2f}x{note}")
...
def main():
    if RUN_ALL_LOO:
        ...
        print(f"\n\n{'='*70}")
        print(f"  [Original TopoAR + baseline(N)]  LOO SUMMARY  ({len(ALL_TOPOS)} configs)")
        print(f"{'='*70}")
        ...
        for r in all_results:
            ...
            print(row)
        print(f"  {'-'*68}")
        print(f"\n  Compare with: python run_experiment.py  (same model, no baseline)")
        print(f"  Difference isolates the effect of baseline(N) preprocessing alone.")
    else:
