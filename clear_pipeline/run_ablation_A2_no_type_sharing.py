#!/usr/bin/env python3
"""Ablation A2: No type-sharing.

Modification: W_DU, K_DU, V_DU, D_DU replaced with per-instance ModuleLists
(max_n=3). All other modules remain shared. Forces a catastrophic failure on
topologies whose DU index was never seen in training, proving type-sharing is
essential for zero-shot transfer.

Run:
    cd clear_pipeline/
    python run_ablation_A2_no_type_sharing.py
"""

ABLATION_NAME = "A2_no_type_sharing"
ABLATION_DESC = "W_DU, K_DU, V_DU, D_DU replaced with per-instance ModuleLists (max_n=3)"
...
def slice_features(z):
    ...
    N_DU = du.shape[1]
    # Baseline topology normalization (kept — A2 only changes the model)
    cu[:, 5] /= N_DU
    cu[:, 6] /= N_DU
    cu = cu[:, CU_FEAT_SLICE]
    du = du[:, :, DU_FEAT_SLICE]
...
def phase_train(fit_streams, train_topos, model_ckpt, model_cls=None):
    ...
    for i, (cu_s, du_s, bid) in enumerate(fit_streams):
        ...
        n = len(ds); perm = rng.permutation(n)
    ...
    # A2: pass max_n to model
    model = model_cls(cu_dim=cu_dim, du_dim=du_dim, embed_dim=EMBED_DIM, max_n=MAX_N).to(DEVICE)
    ...
    torch.save({
        "state_dict": best_state, "cu_dim": cu_dim, "du_dim": du_dim,
        "embed_dim": EMBED_DIM, "max_n": MAX_N, "cal_frac": CAL_FRAC,
        "n_train_rows": n_train_rows, "topos": list(train_topos),
        "preprocess": PREPROCESS_VERSION, "ablation": ABLATION_NAME,
...
def phase_calibrate(cu_sqerr, du_sqerr):
    cu_sqerr_n = cu_sqerr[COLD_START_K:]; du_sqerr_n = du_sqerr[COLD_START_K:]
    n_normal = len(cu_sqerr_n)
    print(f"  Normal-calibration rows: {n_normal}")
    if n_normal < 100:
        raise RuntimeError(f"Too few cal rows: {n_normal}")
    cu_feat_norm = feat_norm_calibrated(cu_sqerr_n)
    du_feat_norm = feat_norm_calibrated(du_sqerr_n.reshape(-1, du_sqerr_n.shape[-1]))
    cu_thr = float(np.percentile(lift_score(cu_sqerr_n, cu_feat_norm), CU_THRESHOLD_PCT))
    du_thr = float(np.percentile(lift_score(du_sqerr_n.reshape(-1, du_sqerr_n.shape[-1]), du_feat_norm), DU_THRESHOLD_PCT))
...
def phase_evaluate(cu_sqerr, du_sqerr, cu_feat_norm, du_feat_norm,
    ...
    def metrics(name, pred, lbl):
        ...
        anom = int((lbl == 1).sum())
        print(f"  {name:<12s}  anom={anom:>6d}  TP={tp:>6d}  FP={fp:>6d}  FN={fn:>6d}  "
...
def _shade(ax, t_array, mask, color, alpha, label=None):
    ...
    for i in range(len(mask)):
        if mask[i] and not in_block:
            block_start = t_array[i]; in_block = True
...
def run_one(train_topos, test_topo):
    ...
    if model_ckpt.exists():
        ...
        if ckpt.get("topos") != list(train_topos):
            mismatches.append(f"topos mismatch")
        if ckpt.get("n_train_rows") != n_fit_total:
            mismatches.append(f"n_train_rows mismatch")
        if mismatches:
            raise SystemExit(f"Incompatible checkpoint {model_ckpt}: {mismatches}\nDelete it and rerun.")
        # A2: load max_n from checkpoint
        model = MODEL_CLS(cu_dim=ckpt["cu_dim"], du_dim=ckpt["du_dim"],
                          embed_dim=ckpt["embed_dim"],
                          max_n=ckpt.get("max_n", MAX_N)).to(DEVICE)
        model.load_state_dict(ckpt["state_dict"])
    else:
        print(f"\n[3] Training {MODEL_CLS.__name__} (max_n={MAX_N}) ...")
