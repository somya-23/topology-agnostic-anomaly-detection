...
CU_FEAT_SLICE = [0, 1, 5, 6]              # cpu, mem_pct, net_tx, net_rx  (4 features, mem_bytes removed)
...
CU_IRATE_IDX = [0, 2, 3]    # cpu, net_tx, net_rx (shifted: mem_bytes removed so net_tx=2, net_rx=3)
...
def run_one(train_topos, test_topo):
    ...
    Returns:     dict mapping entity name → {tp, fp, fn, p, r, f1, anom}
                 e.g. {"CU": {...}, "DU_0": {...}, "ANY": {...}}
    """
    model_ckpt = Path(f"{'bidr_' if BIDIR else ''}no_mem_model_ckpt_test_{test_topo}.pt")   # per-split; delete to retrain
