#!/usr/bin/env python3
"""Ablation A1: No topology normalization.

Modification: removes cu[:, 5] /= N_DU and cu[:, 6] /= N_DU from slice_features.
Everything else — model, training, calibration, inference, evaluation — is identical
to run_experiment.py.

Run:
    cd clear_pipeline/
    python run_ablation_A1_no_topology_normalization.py
"""

ABLATION_NAME = "A1_no_topology_normalization"
ABLATION_DESC = "Removes cu[:, 5] /= N_DU and cu[:, 6] /= N_DU from slice_features"
...
def write_csv(all_metrics, test_topo, train_topos, cu_thr, du_thr):
    ...
    with open(CSV_PATH, "a", newline="") as f:
        ...
        for entity, m in all_metrics.items():
            if entity == "CU":
                thr = f"{cu_thr:.6f}"
            elif entity.startswith("DU"):
                thr = f"{du_thr:.6f}"
            else:
                thr = ""
            f.write(
                f"{ABLATION_NAME},{test_topo},{'+'.join(train_topos)},"
...
def slice_features(z):
    ...
    N_DU = du.shape[1]  # noqa: F841 — kept for reference but NOT used for normalization

    # ABLATION A1: /N_DU normalization lines intentionally absent
    # Baseline has: cu[:, 5] /= N_DU; cu[:, 6] /= N_DU
...
# =============================================================================
# PHASES — identical to run_experiment.py except phase_train accepts model_cls
# =============================================================================
...
def phase_train(fit_streams, train_topos, model_ckpt, model_cls=None):
    ...
    torch.save({
        ...
        "ablation": ABLATION_NAME, "model_cls": model_cls.__name__,
    }, model_ckpt)
...
def phase_calibrate(cu_sqerr, du_sqerr):
    ...
    du_feat_norm = feat_norm_calibrated(du_sqerr_flat)
    cu_thr = float(np.percentile(lift_score(cu_sqerr_n, cu_feat_norm), CU_THRESHOLD_PCT))
    du_thr = float(np.percentile(lift_score(du_sqerr_flat, du_feat_norm), DU_THRESHOLD_PCT))
...
def phase_plot(cu_s_te, du_s_te, cu_stress, du_stress,
    ...
    for row, (name, feat, stress_lbl, scores, pred, thr) in enumerate(entities):
        ...
        ax_f.set_ylabel(name, fontsize=10); ax_f.set_title(f"{name} — features", fontsize=10)
...
def run_one(train_topos, test_topo):
    ...
    if model_ckpt.exists():
        ...
        if ckpt.get("topos") != list(train_topos):
            mismatches.append(f"topos: ckpt={ckpt.get('topos')} vs {list(train_topos)}")
        if ckpt.get("n_train_rows") != n_fit_total:
            mismatches.append(f"n_train_rows: ckpt={ckpt.get('n_train_rows')} vs {n_fit_total}")
        if ckpt.get("cu_dim") != cu_dim:
            mismatches.append(f"cu_dim: ckpt={ckpt.get('cu_dim')} vs {cu_dim}")
        if ckpt.get("du_dim") != du_dim:
            mismatches.append(f"du_dim: ckpt={ckpt.get('du_dim')} vs {du_dim}")
        if mismatches:
            raise SystemExit(f"Incompatible checkpoint {model_ckpt}:\n  " +
                             "\n  ".join(mismatches) +
                             f"\nDelete {model_ckpt} and rerun.")
...
def main():
    if RUN_ALL_LOO:
        ...
        print(f"\n\n{'='*72}")
        print(f"  LEAVE-ONE-OUT SUMMARY  — {ABLATION_NAME}")
        print(f"{'='*72}")
