"""run_experiment_v0_baseline.py — UNMODIFIED v0 pipeline, multi-family driver.

This is a byte-identical copy of run_experiment.py (the original v0 baseline)
with exactly TWO additions so it can run on every stress family for the
comparison matrix without touching the original file:
  1. CLI:  python run_experiment_v0_baseline.py [--base-dir D] [--stress-type N]
  2. CSV:  results go to loo_results_v0_<base-dir>.csv (per family)
Checkpoints are the existing random_model_ckpt_test_*.pt (train data identical
across families). Plots overwrite the v0 NET ones (shared prefix) — the CSVs
are the artifact of record. Method, thresholds, scoring: untouched v0.
...
def main():
    if RUN_ALL_LOO:
        ...
        # Save LOO results to CSV (per family, never collides with originals)
        csv_path = Path(f"loo_results_v0_{BASE_DIR.name}.csv")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--base-dir", default="CU_NET_random_STRESS")
    ap.add_argument("--stress-type", type=int, default=3, choices=[1, 2, 3])
    args = ap.parse_args()
    # Only the dataset pointers change; CKPT_PREFIX stays "random_" (set at
    # import from the default BASE_DIR) so the existing v0 checkpoints are
    # reused — the train data is identical across families.
