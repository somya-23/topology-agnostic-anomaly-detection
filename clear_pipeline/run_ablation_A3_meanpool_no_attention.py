#!/usr/bin/env python3
"""Ablation A3: Mean pooling — no learned attention.

Modification: Q, K_CU, K_DU removed; aggregation replaced with uniform mean
over all entity value-tokens (1/(1+N) weight each). LSTM, decoders, and token
projections are unchanged.

Run:
    cd clear_pipeline/
    python run_ablation_A3_meanpool_no_attention.py
"""

ABLATION_NAME = "A3_meanpool_no_attention"
ABLATION_DESC = "Q/K_CU/K_DU removed; s = mean([V_cu, V_du_0, ..., V_du_N])"
...
def slice_features(z):
    ...
    N_DU = du.shape[1]
    cu[:, 5] /= N_DU; cu[:, 6] /= N_DU  # topology normalization (unchanged from baseline)
